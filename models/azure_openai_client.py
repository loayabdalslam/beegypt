...
class AzureOpenAIClient(BaseAIClient):
    ...
    def generate_text(self, prompt: str, temperature: Optional[float] = None) -> str:
        ...
        try:
            ...
            logger.debug(f"Sending prompt to Azure OpenAI (length: {len(prompt_with_instruction)}):\\n{prompt_with_instruction[:500]}...")
            ...
            logger.info("Making Azure OpenAI API request with rate limiting")
    ...
    def generate_code(self, prompt: str, language: str = "python") -> str:
        ...
        try:
            ...
            # Generate response with rate limiting and retries
            logger.info("Making Azure OpenAI API request for code generation with rate limiting")
            response = self._rate_limiter.execute_with_rate_limit(make_api_call)
    ...
    def analyze_code(self, code: str) -> Dict:
        ...
        try:
            ...
            # Generate response with rate limiting and retries
            logger.info("Making Azure OpenAI API request for JSON generation with rate limiting")
            response = self._rate_limiter.execute_with_rate_limit(make_api_call)
